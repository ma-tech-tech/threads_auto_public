...
| **post-writer** | `generate.sh` (5:00) | Step 2：5 案執筆 | 中間ファイル + 5 タイプ定義 Read | `posts/<D+1>_5案.md`（A〜E のうち 3 タイプ以上 / HEADER_RE 互換） | sonnet | Read のみ（Web 系遮断） |
...
- `channel` は candidate.type から導出（A〜E 型 → `threads_auto`、`seed-*` → `threads_auto_drafts`、`reservation` → `manual`）
...
│           └── 02_5タイプ定義_厚み版.md     # post-writer 専用（git 管理外）
...
- 運用方針：`01_運用方針.md`
- 5 タイプ定義（厚み版）：`.claude/agents/ref/02_5タイプ定義_厚み版.md`（post-writer agent と同居）
